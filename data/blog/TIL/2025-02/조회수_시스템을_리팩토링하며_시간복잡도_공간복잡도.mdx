...
각 Set 키에는 TTL을 적용했지만 일부 게시글은 자주 조회되어 TTL이 계속 갱신되었고, 일부는 조회가 삭제되었다.
...
기존 구조에서는 전체 `게시글(N)을 주기적으로 스캔`하면서, 조회수 변경 여부와 관계없이 모든 게시글에 대해 쓰기 연산을 수행했다.

`트래픽이 증가할수록 불필요한 DB 부하`로 이어졌고, 실제 변경이 없는 게시글에도 `UPDATE 쿼리가 실행되는 문제가 있었다.`

리팩토링 이후에는 Redis Set(`articles:save:db`)을 활용해 조회수 변경이 감지된 게시글(K)만 추적하고,

이들에 대해서만 `일괄 처리(Bulk Update) 방식`으로 DB에 저장함으로써 I/O 부하를 줄일 수 있었다.
...
리팩토링 이후에는 인기글 정렬을 `RDB에서 주기적으로 계산`하고, 그 결과만 `Redis List로 저장`하여, `정렬 로직을 DB에 위임하고 Redis는 단순 캐시 역할만 수행`하도록 변경했다.
