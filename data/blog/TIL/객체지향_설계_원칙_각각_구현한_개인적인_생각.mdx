...
public class DatabaseRepository implements CustomerRepository {
    @Override
    public void save(Customer customer) {
        System.out.println("고객 정보를 데이터베이스에 저장 중입니다: " + customer);
    }
...
- `CustomerService` 클래스는 `CustomerRepository` 인터페이스에 의존하므로, 데이터 저장소가 변경되더라도 `CustomerService` 코드를 수정할 필요가 없다.
- DIP를 적용하여 코드의 확장성이 향상되었다.