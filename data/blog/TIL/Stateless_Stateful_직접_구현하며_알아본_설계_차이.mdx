...
sequenceDiagram
    participant Client
    participant StatelessService

    Client->>StatelessService: 요청 (Action: "로그인")
    StatelessService-->>Client: 응답 ("로그인 처리 완료")

    Client->>StatelessService: 요청 (Action: "대시보드 보기")
    StatelessService-->>Client: 응답 ("대시보드 보기 처리 완료")

    Client->>StatelessService: 요청 (Action: "로그아웃")
    StatelessService-->>Client: 응답 ("로그아웃 처리 완료")
...
sequenceDiagram
    ...
    Client->>StatefulService: 로그인 요청
    StatefulService-->>Client: 응답 ("로그인 성공")
    StatefulService->>StatefulService: 상태 저장 ("로그인 상태")

    Client->>StatefulService: 작업 요청 ("대시보드 보기")
    StatefulService->>StatefulService: 상태 확인 ("로그인 상태")
    StatefulService-->>Client: 응답 ("대시보드 보기 처리 완료")

    Client->>StatefulService: 로그아웃 요청
    StatefulService->>StatefulService: 상태 삭제
    StatefulService-->>Client: 응답 ("로그아웃 성공")
